...
import com.vtu.web.util.gif.AnimatedGifEncoder;
import com.vtu.web.util.gif.ImageUtils;
...
public class MediaUtil {
    ...
    //private static String FFMPEG_PATH = "D:\\soft\\ffmpeg-3.4-win64-static\\bin\\ffmpeg.exe"; // /usr/bin/ffmpeg
    ...
    public static void synthesisPicture(File inFile,File outFile) throws IOException {
        //String dst_pic ="F:\\cc\\test.png";
        //CreateBigImage.changeFolderImages("F:\\cc",600,400);
        File[]  listFiles = inFile.listFiles();
        double sq = Math.sqrt(listFiles.length);
        int ceil = (int)Math.floor(sq);
        int ceilPow = (int) Math.pow(ceil,2);
        BufferedImage[][] images = new BufferedImage[ceil][ceil];
        int[][] ImageArrays = new int[listFiles.length][];
        for(int j = 0; j<ceilPow; j++){
            log.info("正在获取第{}张图片",j);
            int x= ceil-j/ceil-1;
            int y= j%ceil;
            images[x][y]=ImageIO.read(listFiles[j]);
            int width = images[x][y].getWidth();
            int height = images[x][y].getHeight();
            ImageArrays[j]=new int[width * height];
            ImageArrays[j] = images[x][y].getRGB(0, 0, width, height,  ImageArrays[j], 0, width);
        }
        BufferedImage img1= images[0][0];
        int width =img1.getWidth();
        int height = img1.getHeight();
        int bufHeight=0;
        BufferedImage bufferedImage = new BufferedImage(width*ceil,height*ceil,BufferedImage.TYPE_INT_RGB);
        int count=0;
        /*for(int i=0;i<images.length;i++){
            int bugWidth=0;
            BufferedImage[] imageList = images[i];
            for(int j=0;j<imageList.length;j++){
                if(null != imageList[i]){
                    log.info("正在合成第{}张图片",i);
                    bufferedImage.setRGB(bugWidth,bufHeight,width,height,ImageArrays[count],0,width);
                    count++;
                    bugWidth+=width;
                }
            }
        ...
        bufferedImage.setRGB(0,0,width,height,ImageArrays[2],0,width);
        bufferedImage.setRGB(width,0,width,height,ImageArrays[3],0,width);

        bufferedImage.setRGB(0,height,width,height,ImageArrays[0],0,width);
        bufferedImage.setRGB(width,height,width,height,ImageArrays[1],0,width);

        ImageIO.write(bufferedImage, "jpg", outFile);// 写图片 ，输出到硬盘
    }
    ...
    /**
     *
     * @param videoFile 视频输入路径
     * @param logFile 视频输出路径
     * @param resPower 输出分辨率
     * @param endTime 视频时长
     * @param outputPath 输出路径
     */
    public static void getPreviewVido(String videoFile,String logFile,String resPower,String endTime,String outputPath){
        File video = new File(videoFile);
        File log = new File(logFile);
        File out = new File(outputPath);
        getPreviewVido(video,log,resPower,endTime,out);
    }
     ...
     * @param outputPath 输出路径
     */
    public static void getPreviewVido(File videoFile,File logFile,String bitRate,String endTime,File outputPath){
        ...
        commond.add("-i");
        commond.add(logFile.getAbsolutePath());
        commond.add("-b:v");
        commond.add(bitRate);
        /*commond.add("-s");
        commond.add(resPower);*/
        /*commond.add("-t");
        commond.add(endTime);*/
        commond.add("-filter_complex");
        commond.add("overlay=main_w-overlay_w-100:100");
        commond.add("-vcodec");
        commond.add("libx264");
        commond.add(outputPath.getAbsolutePath());
        executeCommand(commond);
    }


    public static void getPreviewVido2(File videoFile,File logFile,File outputPath,VideoMetaInfo videoMetaInfo){
        if (null == videoFile || !videoFile.exists()) {
            throw new RuntimeException("源视频文件不存在，请检查源视频路径");
        }
        if (null == outputPath) {
        ...
        commond.add("-filter_complex");
        if(videoMetaInfo.getWidth()>videoMetaInfo.getHeight()){
            commond.add("[0:v]scale=-1:540[int];[1:v]scale=-1:100[img1];[int][img1]overlay=main_w-overlay_w-100:100");

        }else{
            commond.add("[0:v]scale=540:-1[int];[1:v]scale=-1:100[img1];[int][img1]overlay=main_w-overlay_w-100:100");
        }
    ...
    /**
     * 生成预览图片
     * @param videoFile 视频输入路劲
     * @param outputPath 视频输出路劲
     * @param starFrame 开始帧数
     * @param endFrame 结束帧数
     * @param preFrame 每多少帧截取
     */

    public static void getPreview(String videoFile,String outputPath,Double starFrame,Double endFrame,Double preFrame){
        File video = new File(videoFile);
        File out = new File(outputPath);
        MediaUtil.getPreview(video,out,starFrame,endFrame,preFrame);
     ...
     * @param preFrame 每多少帧截取
     */
    public static void getPreview(File videoFile,File outputPath,Double starFrame,Double endFrame,Double preFrame){
        if (null == videoFile || !videoFile.exists()) {
            throw new RuntimeException("源视频文件不存在，请检查源视频路径");
        }
        if (null == outputPath) {
        ...
        commond.add(videoFile.getAbsolutePath());
        commond.add("-vf");
        commond.add("select=between(n\\,"+starFrame+"\\,"+endFrame+")*not(mod(n\\,"+preFrame+"))");
        commond.add("-vsync");
        commond.add("0");
        commond.add(outputPath+File.separator+"image_%05d.jpg");
        executeCommand(commond);
    }
     ...
     */
    private static void cutVideoFrame(File videoFile, String path, long time, int width, int height, int timeLength, boolean isContinuty) {
        ...
        if (time + timeLength > info.getDuration()/1000) {
        ...
        try {
           ...
           // executeCommand(commond);
            Process videoProcess = new ProcessBuilder(commond).start();
    ...
    /**
     * 开始压缩视频文件
     * @param videoFile 视频文件输入路径
     * @param logFile log路径
     * @param outputFile 视频文件输出路径
     */
    public static void compressVideo(File videoFile,File logFile, File outputFile){
        if (videoFile == null || !videoFile.exists()) {
            throw new RuntimeException("视频文件不存在：");
        }
        if (null == outputFile) {
        ...
        if (null == logFile) {
            throw new RuntimeException("log不存在，请检查log存放路径是否正确");
        }
        if (!outputFile.exists()) {
            /*try {
            ...
            commond.add("400*300");
            commond.add("-filter_complex");
            commond.add("overlay=main_w-overlay_w-10:10");
            commond.add(outputFile.getAbsolutePath());
            executeCommand(commond);
        }
    ...
    public static VideoMetaInfo getVideoMetaInfo(File videoFile) {
        ...
        if (durationMacher.find()) {
            ...
            duration = dec * 10L + seconds * 1000L + minutes * 60L * 1000L + hours * 60L * 60L * 1000L;
            durInfo = durationMacher.group(1)+":"+durationMacher.group(2)+":"+durationMacher.group(3)+"."+durationMacher.group(4);
        ...
        videoMetaInfo.setDurinfo(durInfo);
    ...
    public static void markImageByIcon3(File srcImgPath,File iconPath,
                                       File targerFile) {
        ...
        try {
            ...
            ImageIcon imgIcon = new ImageIcon(iconPath.getAbsolutePath());
     ...
     * @param targerFile 目标图片路径
     */
    public static void markImageByIcon(File srcImgPath,File iconPath,
                                       File targerFile) {
        OutputStream os = null;
        try {
            Image srcImg = ImageIO.read(srcImgPath);
            int ImageWidth = srcImg.getWidth(null);
            int imageHeight = srcImg.getHeight(null);
            double scale = (double)1000/ImageWidth;
            BufferedImage buffImg = new BufferedImage(srcImg.getWidth(null),
                    srcImg.getHeight(null), BufferedImage.TYPE_INT_RGB);
            buffImg = ImageUtils.modifyImageScale(buffImg,scale);
            ...
            g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
                    RenderingHints.VALUE_INTERPOLATION_BILINEAR);
            g.drawImage(srcImg.getScaledInstance(buffImg.getWidth(), buffImg.getHeight(), Image.SCALE_SMOOTH), 0, 0, null);
            //g.drawImage(srcImg.getScaledInstance(ImageWidth, imageHeight, Image.SCALE_SMOOTH), 0, 0, null);
            // 水印图象的路径 水印一般为gif或者png的，这样可设置透明度
            ImageIcon imgIcon = new ImageIcon(iconPath.getAbsolutePath());
            ...
            // 表示水印图片的位置
            //g.drawImage(buffImg, 0,0,w,h, null);
            g.drawImage(img, 650, 100,266,100, null);
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER));
            g.dispose();
            os = new FileOutputStream(targerFile);
            // 生成图片
    ...
    public static void markImageByIcon2(File srcImgPath,File iconPath,
                                        File targerFile,Integer location) {
        OutputStream os = null;
        try {
            Image srcImg = ImageIO.read(srcImgPath);
            ...
            log.info("图片完成添加Icon印章... srcImgPath:{}, targerPath:{}", srcImgPath, targerFile.getAbsolutePath());
        } catch (Exception e) {
            log.info("图片添加Icon印章异常！ iconPath:{}, srcImgPath:{}, targerPath:{}, location:{}", iconPath, srcImgPath, targerFile, location);
        } finally {
            try {
                if (null != os) {
                    os.close();
